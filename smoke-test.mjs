#!/usr/bin/env node
/* BRIDGE smoke test — confirms the three services are up and talking.
 *
...
async function main() {
  console.log("\n\x1b[1mBRIDGE smoke test\x1b[0m");
  ...
  // 2. gameserver health
  {
    ...
    else { const t = await r.text(); t.trim() === "ok" ? ok(`game server healthy at ${GAME}`) : no(`game server /health returned: ${t.slice(0, 40)}`); }
  }
  // 3. client (optional)
  {
  ...
  let token = null;
  {
    const r = await tryFetch(() => post(BACKEND, "/auth/google", { name: "SmokeTest", email: "smoke@test.local", mockGoogleId: "smoke-1" }));
