    ...
    <div style={wrap}>
      ...
      <div style={{ position: "relative", zIndex: 2, padding: "32px 40px", height: "100%", overflowY: "auto" }}>
        ...
        <div className="row" style={{ alignItems: "flex-end", gap: 28, marginBottom: 8, flexWrap: "wrap" }}>
          <div>
            <div className="tag"><span>Pilot dossier</span></div>
            <div className="row" style={{ alignItems: "flex-end", gap: 18, marginTop: 10 }}>
              <div className="kanji" style={{ fontSize: 26, color: "var(--volt)", lineHeight: 1, paddingBottom: 18 }}>階級</div>
              <div>
                <div className="impactf dim" style={{ letterSpacing: "0.2em", fontSize: 13 }}>RANK</div>
              ...
              <div style={{ paddingBottom: 22 }}>
                ...
                <div className="dim" style={{ fontWeight: 600 }}>{xp.toLocaleString()} XP banked</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 720, marginBottom: 40 }}>
          <div className="row" style={{ justifyContent: "space-between", marginBottom: 6 }}>
            <span className="impactf" style={{ fontSize: 12, letterSpacing: "0.15em", color: "var(--volt)" }}>LV {level}</span>
            <span className="impactf dim" style={{ fontSize: 12 }}>{xpToNext.toLocaleString()} XP TO LV {level + 1}</span>
          </div>
          <div style={river}>
        ...
        {/* UNLOCK LADDER */}
        <div className="tag" style={{ marginBottom: 16 }}><span>Unlock path</span></div>
              ...
              <div key={row.lvl} className="panel" style={{ ...ladderCard, ...(reached ? cardReached : isNext ? cardNext : null) }}>
                ...
                <div className="col gap-s" style={{ marginTop: 8 }}>
                  {(row.grants || []).map((id) => {
                    ...
                    return (
                      <div key={id} className="row gap-s" style={{ fontSize: 12, fontWeight: 600, opacity: reached ? 1 : 0.55 }}>
                        <span style={{ width: 6, height: 6, background: rarityColor(c?.rarity), display: "inline-block" }} />
                        <span>{c?.name || id}</span>
                      </div>
                    );
                  })}
                  {(row.slots || []).map((s) => (
                    <div key={s} className="faint" style={{ fontSize: 11, letterSpacing: "0.08em" }}>+ {s} slot</div>
                  ))}
                  {(row.perks || []).map((p) => (
                    <div key={p} style={{ fontSize: 11, color: "var(--violet)", fontWeight: 700 }}>◆ {p.replace(/_/g, " ")}</div>
                  ))}
                </div>
                {isNext && <div style={nextFlag} className="impactf">NEXT</div>}
              </div>
            );
          })}
        </div>
...
function Loading() {
  return <div style={{ ...wrap, display: "grid", placeItems: "center" }}><div className="display dim" style={{ fontSize: 40 }}>LOADING…</div></div>;
}
