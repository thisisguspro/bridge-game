...
export default function Play({ user, profile, onMatchActiveChange }) {
  ...
  useEffect(() => {
    ...
    if (ph !== prevPhase.current) {
      ...
      if (ph === "ended") {
        const won = view.winner === "crew" ? "THE POSSE PREVAILS" : "OUTLAWS WIN";
    ...
    <div style={{ height: "100%", display: "grid", gridTemplateColumns: "300px 1fr 320px", ...shakeFx.style }} onAnimationEnd={shakeFx.onAnimationEnd}>
      ...
      {!panelOpen ? (
        ...
        <div style={{ ...sidePane, width: 92, padding: "16px 8px", display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
          ...
          <NumStat label="CHARGE" value={Math.round(you.oxygen)} max={100} color="var(--gold)" />
      ...
      <div style={sidePane}>
        ...
        <Gauge label="BATTERY" value={Math.round(you.oxygen)} max={100} color="var(--gold)" kanji="電池" />
        ...
        <div style={{ marginTop: 16 }}>
          ...
          <div className="row gap-s" style={{ alignItems: "center" }}>
            <Sys on={view.systems?.oxygenOn} label="PWR" />
      ...
      <div style={{ position: "relative", overflow: "hidden" }}>
        ...
        {onEnergy && (
          <>
            ...
            <div style={{ position: "absolute", top: 130, left: "50%", transform: "translateX(-50%)", textAlign: "center", pointerEvents: "none" }}>
              <div className="kanji" style={{ fontSize: 18, color: "var(--volt)", letterSpacing: "0.3em" }}>GHOST RIDER</div>
              <div className="impactf" style={{ fontSize: 13, color: "var(--volt)", letterSpacing: "0.12em" }}>REPAIR DRONE — STILL IN THE FIGHT</div>
              <div className="faint" style={{ fontSize: 11, marginTop: 2 }}>You're a maintenance drone now — fix things to help the posse. The living can't see or hear you. Get zapped again and you're scrap.</div>
            </div>
          </>
        )}

        {/* top-left role + car readout */}
        <div style={{ position: "absolute", top: 16, left: 20, pointerEvents: "none" }}>
          <div className="kanji" style={{ fontSize: 14, color: onEnergy ? "var(--volt)" : isImpostor ? "var(--violet)" : "var(--gold)" }}>{onEnergy ? "DRONE" : isImpostor ? "WANTED" : "POSSE"}</div>
          <div className="display" style={{ fontSize: 40, lineHeight: 0.85 }}>{room || "—"}</div>
          <span style={{ ...roleBadge, fontSize: 11, padding: "4px 10px", borderColor: onEnergy ? "var(--volt)" : isImpostor ? "var(--violet)" : "var(--gold)", color: onEnergy ? "var(--volt)" : isImpostor ? "var(--violet)" : "var(--gold)" }}>
            {onEnergy ? "DOWNED · DRONE" : isImpostor ? "OUTLAW" : "FOLK"}
          </span>
        </div>
        ...
        <div style={{ position: "absolute", top: 14, right: 18, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
          <div className="impactf faint" style={{ fontSize: 10, letterSpacing: "0.12em", pointerEvents: "none" }}>WASD MOVE · E INTERACT · Z EMOTE · V VOTE <span style={{ color: "var(--gold)" }}>· build R16</span></div>
          <div className="row gap-s">
        ...
        {((map.refillRooms || []).includes(room) || (map.repairRooms || []).includes(room) || isImpostor) && (
          <div style={hudBar}>
            {(map.refillRooms || []).includes(room) && <button className="btn" style={hudBtn} onClick={act(() => conn.refill(roomId))}>Recharge</button>}
          ...
          <div className="row gap-s" style={{ flexWrap: "wrap", marginBottom: 28 }}>
            {impostors.map((p) => (
              <div key={p.id} className="panel panel-hot" style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px" }}>
                ...
                <span className="kanji" style={{ color: "var(--violet)", fontSize: 14 }}>WANTED</span>
              </div>
            ))}
          </div>
