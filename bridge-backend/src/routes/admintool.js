...
adminToolRouter.post("/store/item", requireAdminRole, async (req, res) => {
  try { res.json({ item: await db.adminCreateStoreItem(req.body || {}) }); }
  catch (e) { res.status(400).json({ error: e.message }); }
});
adminToolRouter.post("/store/:id", requireAdminRole, async (req, res) => {
  try { res.json({ entry: await db.adminUpdateStoreEntry(req.params.id, req.body || {}) }); }
  catch (e) { res.status(400).json({ error: e.message }); }
});
adminToolRouter.delete("/store/:id", requireAdminRole, async (req, res) => {
  res.json({ deleted: await db.adminDeleteStoreEntry(req.params.id) });
});
...
adminToolRouter.post("/users/:id/grant", requireAdminRole, async (req, res) => {
  ...
  const result = {};
  try {
    if (cosmeticId) result.cosmetic = await db.grantCosmetic(req.params.id, cosmeticId);
